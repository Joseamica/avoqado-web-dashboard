...
2. **Add translation keys** for required languages (en, es)
...
6. **Test in required languages** before considering work complete

### Task Completion Checklist

- [ ] **No hardcoded text**: All strings use `t('...')` - zero exceptions
- [ ] **Multilingual support**: Keys added for en and es
...
- [ ] **User testing**: Interface verified in required languages
...
**Fix**: Add key to required language files (en, es)
...
**4. Optional: Add French translations (if maintaining fr):**
```json
...
**6. Test in required languages:**
- Switch language in UI
- Verify all strings display correctly
- Check for layout issues (translations can be longer than English)
...
5. [ ] **Optional French translations** - Verify quality if added
...
9. [ ] **Test in required languages** - Verify display in en, es (fr if updated)

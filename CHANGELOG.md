...
- **`PayoutList` action menu items (process / complete / cancel) always hidden due to phantom permission gate** (`PayoutList.tsx:224, 232, 242`). The `PermissionGate permission="commissions:process_payout"` referenced a string that doesn't exist in backend's permission catalog — the canonical perm for OWNER-level payout actions is `commissions:payout` (used by `dashboard/commission.routes.ts` and listed in `DEFAULT_PERMISSIONS[OWNER]`). Result: even OWNERs saw an empty action menu on every payout row. Fix: replaced all 3 occurrences with the canonical `commissions:payout`. UI-only change, no backend impact.

- **`RolePermissions` "Crear conjunto de permisos" button hidden for ADMIN/OWNER due to phantom permission gate** (`RolePermissions.tsx:171`). The `PermissionGate permission="role-permissions:update"` referenced a string that does not exist anywhere in `avoqado-server/src/lib/permissions.ts` — not in `DEFAULT_PERMISSIONS`, `PERMISSION_DEPENDENCIES`, nor `INDIVIDUAL_PERMISSIONS_BY_RESOURCE`. Only SUPERADMIN passed via the `*:*` short-circuit; every other role saw the button hidden. The backend exhibited the same bug in 3 of the 4 permission-set routes in `dashboard.routes.ts` (POST create, PUT update, POST duplicate) — the DELETE route (line 7986) already used the correct `settings:manage`, which made the inconsistency obvious. Fix: changed both the frontend gate and the 3 backend route gates to `settings:manage`, the canonical permission for role/permission administration that already exists in `DEFAULT_PERMISSIONS` for ADMIN/OWNER/SUPERADMIN. Zero impact on production today because no PermissionSets exist in the entire platform (0 rows across 59 venues), but unblocks the feature for ADMIN/OWNER going forward.

- **`PermissionEditorModal` could wipe a role's custom permissions when only the display name/color was edited under slow network** (`PermissionEditorModal.tsx:88, 305-313, 347`). Two race conditions: (a) `useQuery` for `rolePermissionsData` starts undefined, so `modifiedPermissions` initializes at `[]`; if the user opens the modal with slow network and clicks Save before the query resolves, the mutation fires with `[]` → backend upserts a `VenueRolePermission` row with empty permissions → role loses all access. (b) `handleSave` always invoked `updateMutation.mutate()` even when only `hasConfigChanges` was true (appearance-only edit), so editing just the role's display name or color would silently overwrite permissions with whatever stale local state happened to hold. Compounded by (c) — `['*:*']` is read-expanded into a literal list of every permission in `PERMISSION_CATEGORIES`; if the user toggled anything (or even closed and reopened), the save logic only re-collapsed back to `['*:*']` when the full list matched exactly, so any drift between dashboard catalog and stored set silently downgraded the wildcard to literals. Fix: (1) added `isLoadingPermissions` from useQuery and extended the Save button's `disabled` to block clicks while `hasChanges && isLoadingPermissions`; (2) split the appearance and permissions save paths inside `handleSave` so the permissions mutation only fires when the user actually toggled a permission (`hasChanges === true`), and appearance-only saves just close the modal after the config update succeeds. Defensive in depth — none of the 444 active staff across 58 default-role venues are affected because they have no custom `VenueRolePermission`, but Mindform (the only venue with customs) was one connection drop away from a role-wipe incident.

- **`PermissionDetailPanel` category-level toggle cleared all permissions when partially active instead of filling the rest** (`PermissionDetailPanel.tsx:363-377`). The previous logic branched on `hasAnyEnabled` (any > 0), collapsing the natural tri-state `none / partial / all` into a binary `off / on` toggle. So a user with 3 of 5 permissions enabled in a category, clicking the switch hoping to enable the remaining 2, instead lost the 3 they had. Replaced with a proper tri-state: `none → enable all`, `partial → fill the remaining`, `all → clear all` (still respecting `CRITICAL_PERMISSIONS` for own-role lockout protection). Visual indicator (`hasAnyEnabled` driving the switch checked state) preserved for continuity — the bug was in the click handler, not the indicator.

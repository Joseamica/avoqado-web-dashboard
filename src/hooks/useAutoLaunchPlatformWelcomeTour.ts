...
export function useAutoLaunchPlatformWelcomeTour(): { start: () => void; cancel: () => void } {
  ...
  useEffect(() => {
    ...
    // The guided DEMO journeys (avoqado.io/demo → ?demoTour=...) take priority over the
    // welcome tour: a visitor arriving from the marketing tour must land straight in their
    // journey's driver tour, not the generic onboarding. The param check covers the first
    // render; the sessionStorage stash/flags cover the rest of the tab session.

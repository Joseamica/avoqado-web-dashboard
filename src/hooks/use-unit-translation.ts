...
export function useUnitTranslation() {
   ...
   */
  const formatUnit = (unitEnum: string): string => {
    if (!unitEnum) return ''
   ...
   */
  const getShortLabel = (unitEnum: string): string => {
   ...
   */
  const getFullName = (unitEnum: string): string => {
   ...
   */
  const formatUnitWithQuantity = (quantity: number, unitEnum: string): string => {
    if (!unitEnum) return ''

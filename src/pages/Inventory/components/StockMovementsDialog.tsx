...
export function StockMovementsDialog({ open, onOpenChange, rawMaterial }: StockMovementsDialogProps) {
  ...
  const { data: movements, isLoading } = useQuery({
    queryKey: ['stockMovements', venueId, rawMaterial?.id],
    queryFn: async () => {
      if (!rawMaterial) return []
      const response = await rawMaterialsApi.getMovements(venueId, rawMaterial.id, { limit: 50 })
      return response.data.data as Movement[]
    },
                ...
                return (
                  ...
                  >
                    ...
                    {index < movements.length - 1 && (
                      <div className="absolute left-[26px] top-[60px] w-0.5 h-[calc(100%+12px)] bg-border" />
                    )}

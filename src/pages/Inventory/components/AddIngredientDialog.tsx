...
export function AddIngredientDialog({ open, onOpenChange, product, mode, onAddTempIngredient }: AddIngredientDialogProps) {
  ...
  const estimatedCost = selectedRawMaterial && watch('quantity') ? Number(selectedRawMaterial.costPerUnit) * Number(watch('quantity')) : 0
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('rawMaterials.fields.currentStock')}: {Number(selectedRawMaterial.currentStock).toFixed(2)}{' '}
                        {formatUnit(selectedRawMaterial.unit)} · {Currency(Number(selectedRawMaterial.costPerUnit))} /{' '}
                  ...
                  {materialsLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 space-y-3">
                      ...
                      {debouncedSearchTerm.trim().length > 0 && (
                        <Button type="button" variant="outline" size="sm" onClick={() => setQuickCreateOpen(true)} className="gap-1">
            ...
            {selectedRawMaterial && (
              <>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox id="isOptional" checked={isOptional} onCheckedChange={checked => setValue('isOptional', checked as boolean)} />
                ...
                <div className="space-y-3 p-4 rounded-lg border border-border bg-muted/30">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t(
                          'recipes.ingredients.variableIngredientDesc',
                          'Allow customers to substitute this ingredient via modifier selection',
                        )}
                      </p>
                    </div>
                    <Switch id="isVariable" checked={isVariable} onCheckedChange={checked => setValue('isVariable', checked as boolean)} />
                  </div>
                  ...
                  {isVariable && (
                    <div className="space-y-2 pt-2">
                        ...
                        <SelectContent>
                          {modifierGroupsLoading ? (
                              ...
                              <SelectItem key={group.id} value={group.id}>
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                    {group.modifiers?.filter(m => m.inventoryMode === 'SUBSTITUTION').length || 0}{' '}
                                    {tMenu('modifiers.inventory.modeSubstitution', 'substitution')}{' '}
                                    {t('recipes.ingredients.modifiers', 'modifiers')}
                                  </span>
                                </div>
                              </SelectItem>
                            ))
                            ...
                            <div className="px-2 py-4 text-sm text-muted-foreground text-center">
                              {t(
                                'recipes.ingredients.noSubstitutionGroups',
                                'No modifier groups with substitution modifiers found. Create modifiers with SUBSTITUTION inventory mode first.',
                              )}
                            </div>
                          )}
                        </SelectContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t(
                          'recipes.ingredients.linkedModifierGroupDesc',
                          'When a customer selects a modifier from this group, it will replace this ingredient',
                        )}
                      </p>
                    </div>
                  )}

...
export function CreateRecipeWizard({ open, onClose }: CreateRecipeWizardProps) {
  ...
  // Map for O(1) raw material lookups (used in cost calc and ingredient display)
  const rawMaterialMap = useMemo(() => new Map(rawMaterialsData?.map(rm => [rm.id, rm]) ?? []), [rawMaterialsData])
  ...
  }, [ingredients, rawMaterialMap, selectedProduct, portionYield])
      ...
      <FullScreenModal
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {currentStep < 3 ? (
              <Button onClick={handleNext} disabled={!canGoNext() || createRecipeMutation.isPending} className="h-10 px-6">
                {tCommon('next')}
                <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
            ) : (
              <Button onClick={handleCreateRecipe} disabled={createRecipeMutation.isPending} className="h-10 px-6 gap-2">
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-8">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">{t('recipeWizard.stepProgress', { current: currentStep, total: 3 })}</span>
            ...
            <div className="flex justify-between">
              {[1, 2, 3].map(step => (
                <div key={step} className="flex items-center gap-2">
                  ...
                  <span className={cn('text-sm hidden sm:inline', currentStep >= step ? 'text-foreground' : 'text-muted-foreground')}>
              ...
              {selectedProduct && (
                <div className="flex items-center gap-4 p-5 rounded-xl border border-primary/30 bg-primary/5">
                  {selectedProduct.imageUrl ? (
                    <img src={selectedProduct.imageUrl} alt={selectedProduct.name} className="w-12 h-12 rounded-xl object-cover shrink-0" />
                  ) : (
            ...
            <div className="space-y-8">
              ...
              <section className="bg-card rounded-2xl border border-border/50 p-6">
                ...
                <SearchCombobox
                  ...
                  onSelect={item => {
                    const material = rawMaterialMap.get(item.id)
                    if (material) {
                ...
                {pendingMaterial && (
                  <div className="mt-3 rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
                    ...
                    {pendingQuantity && Number(pendingQuantity) > 0 && (
                      <p className="text-xs text-muted-foreground">
                        {t('recipes.ingredients.cost')}:{' '}
                        <span className="font-semibold text-foreground">
                          {Currency(Number(pendingMaterial.costPerUnit) * Number(pendingQuantity))}
                        </span>
                      </p>
                    )}
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">
                              ...
                              {rawMaterial && <> × {Currency(costPerUnit)}</>}
                            </p>
                          </div>
                          <span className="text-sm font-semibold tabular-nums shrink-0">{Currency(lineCost)}</span>
                          <button
                            type="button"
      ...
      <ProductWizardDialog
        ...
        onSuccess={async (productId: string) => {
          try {
            ...
            const product = response.data.data as ProductWithRecipe
            setSelectedProduct(product)
            setSearchTerm('')
            queryClient.invalidateQueries({ queryKey: ['products-without-recipes', venueId] })
          } catch {
            // Product created but couldn't fetch — user can search for it
            queryClient.invalidateQueries({ queryKey: ['products-without-recipes', venueId] })
          }
        }}
      />

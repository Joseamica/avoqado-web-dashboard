    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-lg px-4 py-6">
        ...
        <div className="mb-6 flex justify-center">
          <Badge className={statusColors[reservation.status] || ''}>{t(`status.${reservation.status}`)}</Badge>
        </div>
        ...
        {isCancelled && (
          <div className="mb-6 rounded-xl bg-red-50 p-4 text-center dark:bg-red-900/10">
            ...
            <p className="font-medium text-red-700 dark:text-red-400">{t('publicBooking.manage.cancelledStatus')}</p>
          </div>
        )}
        ...
        <div className="space-y-4 rounded-xl border border-border p-4">
          ...
          {/* Product / Service(s) — list ALL booked services (multi-service
					    appointments), falling back to the single lead service. */}
          {(() => {
            const services =
              reservation.services && reservation.services.length > 0
                ? reservation.services
                : reservation.product
                  ? [{ id: reservation.product.id, name: reservation.product.name }]
                  : []
          ...
          {reservation.table && (
            <div className="flex items-center gap-3">
              ...
              <p>
                {t('publicBooking.manage.table')} {reservation.table.number}
              </p>
            </div>
          )}
          ...
          {reservation.assignedStaff && (
            <div className="flex items-center gap-3">
              ...
              <p>
                {t('publicBooking.manage.staff')}: {reservation.assignedStaff.firstName} {reservation.assignedStaff.lastName}
              </p>
            </div>
          )}
        ...
        <div className="mt-6 space-y-3">
          {!isCancelled && reservation.status !== 'COMPLETED' && reservation.status !== 'NO_SHOW' && (
            <Button variant="destructive" className="h-11 w-full" onClick={() => setShowCancelDialog(true)}>
              {t('publicBooking.manage.cancelButton')}
            </Button>
          )}

          <Button variant="outline" className="h-11 w-full" onClick={() => navigate(`/book/${venueSlug}`)}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            {t('publicBooking.manage.backToBooking')}
          </Button>
        </div>
      ...
      <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t('publicBooking.manage.cancelDescription')}</AlertDialogDescription>
          </AlertDialogHeader>

...
export const TeamHealthChart: React.FC<TeamHealthChartProps> = ({
  managerId,
  className,
}) => {
  ...
  const healthyPercentage = Math.round(
    ((MOCK_DATA[0].value + MOCK_DATA[1].value) / totalPromoters) * 100
  )
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold">
            {t('playtelecom:managers.teamHealth', { defaultValue: 'Salud del Equipo' })}
          </h3>
        ...
        <div className="text-right">
          <p className={cn(
            'text-2xl font-bold',
            healthyPercentage >= 70 ? 'text-green-600 dark:text-green-400' :
            healthyPercentage >= 50 ? 'text-yellow-600 dark:text-yellow-400' :

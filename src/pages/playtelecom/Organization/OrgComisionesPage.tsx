/**
 * OrgComisionesPage — Org-level Comisiones (Cash Out) for PlayTelecom-style organizations.
 *
 * Org-wide version of playtelecom/Comisiones/ComisionesPage.tsx. Back-office config +
 * aggregated payout view for the same-day promoter commission scheme, scoped to the
 * WHOLE organization (all venues) instead of a single venue:
 *  - Escalated rate table per sale type (Línea Nueva / Portabilidad) — org default.
 *  - Active-days calendar (which days the scheme is on) — org-wide.
 *  - Aggregated withdrawals (retiros) across all venues + "Generar reporte" for Finanzas.
 *
 * Per-promoter saldo / "Retirar" controls are intentionally NOT here — withdrawals are
 * created at the venue/TPV level. This page is read/aggregate + config only.
 *
 * Gated by OwnerProtectedRoute (route guard). Amounts are PESOS.
 * Backend: /api/v1/dashboard/organizations/:orgId/cash-out/*.
 */
...
export default function OrgComisionesPage() {
  ...
  const ratesQuery = useQuery({
    queryKey: ['cash-out-org-rates', orgId],
    queryFn: () => cashOutService.getOrgRates(orgId as string),
    enabled: !!orgId,
  })
  ...
  const saveRates = useMutation({
    mutationFn: () => cashOutService.saveOrgRates(orgId as string, rates),
    onSuccess: () => {
      toast({ title: t('comisiones.ratesSaved', { defaultValue: 'Tarifas guardadas' }) })
      setDraft(null)
      qc.invalidateQueries({ queryKey: ['cash-out-org-rates', orgId] })
    },
    onError: fail,
  })

  // ---------- Active days (org-wide) ----------
  const daysQuery = useQuery({
    queryKey: ['cash-out-org-days', orgId],
    queryFn: () => cashOutService.getOrgActiveDays(orgId as string),
    enabled: !!orgId,
  })
  ...
  const saveDays = useMutation({
    mutationFn: () => cashOutService.saveOrgActiveDays(orgId as string, activeDays),
    onSuccess: () => {
      toast({ title: t('comisiones.daysSaved', { defaultValue: 'Días guardados' }) })
      setDaysDraft(null)
      qc.invalidateQueries({ queryKey: ['cash-out-org-days', orgId] })
    },
    onError: fail,
  })

  // ---------- Withdrawals (aggregated across venues) + report ----------
  const withdrawalsQuery = useQuery({
    queryKey: ['cash-out-org-withdrawals', orgId],
    queryFn: () => cashOutService.listOrgWithdrawals(orgId as string),
    enabled: !!orgId,
  })
  ...
  const genReport = useMutation({
    mutationFn: () => cashOutService.generateOrgReport(orgId as string),
    onSuccess: r => {
      toast({
      ...
      qc.invalidateQueries({ queryKey: ['cash-out-org-withdrawals', orgId] })
    },
    onError: fail,
  })
  ...
  return (
    <div className="space-y-6">
      <div>
        <PageTitleWithInfo
          title={`${t('comisiones.title', { defaultValue: 'Comisiones' })}${organization?.name ? ` · ${organization.name}` : ''}`}
          className="text-xl font-bold tracking-tight"
          tooltip={t('comisiones.orgTooltip', {
            defaultValue: 'Configuración y reporte de comisiones a nivel organización. Los retiros se generan desde cada sucursal/TPV.',
          })}
        />
        <p className="mt-1 text-sm text-muted-foreground">
          {t('comisiones.orgSubtitle', { defaultValue: 'Tarifas, calendario y retiros agregados de todas las sucursales.' })}
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-left text-[11px] uppercase text-muted-foreground">
                <tr className="border-b border-input">
                  ...
                  <th className="py-2 pr-3 font-medium">{t('comisiones.branch', { defaultValue: 'Sucursal' })}</th>
              ...
              <tbody>
                {withdrawals.map(w => (
                  <tr key={w.id} className="border-b border-input/50">
                    ...
                    <td className="py-2 pr-3">{w.venueName ?? '—'}</td>

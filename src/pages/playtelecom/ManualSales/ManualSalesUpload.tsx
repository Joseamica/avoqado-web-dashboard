 ...
 *
 * Access is gated by the route's <OwnerProtectedRoute> (OWNER/SUPERADMIN) — NOT by an
 * in-component <PermissionGate>. On /organizations/:orgId routes there is no active
 * venue, so useAccess().can() is disabled (use-access.ts: `enabled: !!venueId`) and
 * returns false for EVERYONE (incl. SUPERADMIN); a venue-scoped PermissionGate here
 * would render null and blank the whole page. This matches the sibling org pages
 * (OrgStockControlPage.tsx / OrgComisionesPage.tsx, which gate by role / route guard).
 * orgId comes from useCurrentOrganization(). The backend still enforces
 * `manual-sales:create` on the preview/apply endpoints.
 */

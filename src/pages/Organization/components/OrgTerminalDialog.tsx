...
interface OrgTerminalDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  orgId: string
  terminal?: OrgTerminal | null
  onSave: (data: CreateOrgTerminalRequest | UpdateOrgTerminalRequest) => Promise<void>
}

export const OrgTerminalDialog: React.FC<OrgTerminalDialogProps> = ({
  open,
  onOpenChange,
  orgId,
  terminal,
    ...
    try {
      if (terminal) {
        await onSave({
          ...
          assignedMerchantIds: formData.assignedMerchantIds,
        } as UpdateOrgTerminalRequest)
      } else {
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <div className="flex items-center gap-2">
              ...
              <DialogTitle>
                {terminal ? t('terminals.dialog.editTitle') : t('terminals.dialog.createTitle')}
              </DialogTitle>
            </div>
            <DialogDescription>
              {terminal ? t('terminals.dialog.editDescription') : t('terminals.dialog.createDescription')}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label>
                {t('terminals.dialog.serialNumber')} <span className="text-destructive">*</span>
              </Label>
              <Input
                value={formData.serialNumber}
                onChange={e => setFormData({ ...formData, serialNumber: e.target.value })}
                placeholder={t('terminals.dialog.serialPlaceholder')}
                required={!terminal}
                disabled={!!terminal}
                className="bg-background border-input font-mono"
              />
              <p className="text-xs text-muted-foreground">{t('terminals.dialog.serialHelp')}</p>
            </div>

            <div className="grid gap-2">
              <Label>
                {t('terminals.dialog.name')} <span className="text-destructive">*</span>
              </Label>
              <Input
                value={formData.name}
                onChange={e => setFormData({ ...formData, name: e.target.value })}
                placeholder={t('terminals.dialog.namePlaceholder')}
                required
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="grid gap-2">
                <Label>{t('terminals.dialog.type')}</Label>
                <Select
                  value={formData.type}
              ...
              <div className="grid gap-2">
                <Label>{t('terminals.dialog.brand')}</Label>
                <Input
              ...
              <div className="grid gap-2">
                <Label>{t('terminals.dialog.model')}</Label>
                <Input
            ...
            {!terminal && (
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4"
                />
                <span className="text-sm">{t('terminals.dialog.generateCode')}</span>
              </label>
            )}
            ...
            {formData.assignedMerchantIds.length === 0 && (
              <div className="flex items-start space-x-2 text-sm bg-yellow-50 dark:bg-yellow-950/50 p-3 rounded-md border border-yellow-200 dark:border-yellow-800">
                ...
                <p className="text-yellow-700 dark:text-yellow-300">{t('terminals.dialog.noMerchantWarning')}</p>
              </div>
            )}
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              ...
              {loading
                ? t('terminals.dialog.saving')
                : terminal
                  ? t('terminals.dialog.update')
                  : t('terminals.dialog.create')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>

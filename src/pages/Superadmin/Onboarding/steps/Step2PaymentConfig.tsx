    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="font-semibold">Configuracion de Pagos</h3>
            <p className="text-sm text-muted-foreground">Tasas, comisiones y cuenta merchant</p>
          </div>
        </div>
        ...
        {(organizationId || isNewOrg) && (
          <div className="space-y-4 mt-4 rounded-2xl border border-border/50 p-4">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => onChange({ useOrgConfig: !pricing.useOrgConfig, createOrgConfig: undefined })}>
              <Switch
                ...
                {!orgHasConfig && !orgStatusLoading && (
                  <div className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4 space-y-4">
                    ...
                    {!pricing.createOrgConfig?.primaryAccountId && merchantAccounts.length > 0 && (
                      <Badge variant="outline" className="rounded-full bg-orange-500/10 text-orange-600">
      ...
      {/* Rate inputs — shown when NOT using org config, or when creating new org config */}
        ...
        <div className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Porcentajes por tipo de tarjeta</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
                ...
                <div className="grid grid-cols-4 gap-3 text-center text-xs">
                  {[
                    ...
                    <div key={r.label}>
                      ...
                      <p className="font-semibold text-green-600 dark:text-green-400">+{(r.ours - r.theirs).toFixed(2)}%</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Merchant account selector — venue-level path */}
            {!pricing.useOrgConfig && (
              <div className="space-y-3">
                {pricing.merchantAccountId && (
                  <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-3">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-green-600 dark:text-green-400" />
                      <p className="text-sm font-medium">Merchant account vinculado</p>
                    </div>

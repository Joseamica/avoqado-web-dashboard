...
export interface Reservation {
  ...
  /** Ordered list of product ids for multi-service appointments (Square
   *  pattern): lead service in `productId`, full list here. */
  productIds?: string[]
  /** Resolved services for the reservation, in booking order. The backend
   *  resolves these from `productIds` (which is a scalar text[], not a
   *  relation) so the detail view can list EVERY booked service — not just
   *  the lead `product`. Empty/undefined for table-only reservations. */
  services?: Array<{
    id: string
    name: string
    price: number | null

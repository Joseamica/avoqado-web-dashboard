...
const SIDEBAR_WIDTH_DEFAULT = 256 // 16rem = 256px
  ...
  return (
    ...
    >
      ...
      >
        <div
          data-sidebar="sidebar"
          className="flex h-full w-full flex-col bg-sidebar font-sans antialiased group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<'button'>>(({ className, ...props }, ref) => {
  const { toggleSidebar, sidebarWidth, setSidebarWidth, state, isMobile } = useSidebar()
  const { t } = useTranslation()
  ...
  const handleMouseDown = React.useCallback(
    (e: React.MouseEvent) => {
      // Don't handle drag on mobile or when collapsed
      if (isMobile) return
      ...
      const sidebarWrapper = (e.target as HTMLElement).closest('[data-side]')
      ...
      const sidebarEl = document.querySelector("[data-sidebar='sidebar']")?.parentElement
      const gapEl = sidebarEl?.previousElementSibling as HTMLElement | null
      if (sidebarEl) (sidebarEl as HTMLElement).style.transitionDuration = '0ms'
      if (gapEl) gapEl.style.transitionDuration = '0ms'

      const handleMouseMove = (moveEvent: MouseEvent) => {
        const deltaX = moveEvent.clientX - startXRef.current
        const totalDrag = Math.abs(deltaX)
        ...
        if (isDraggingRef.current) {
          // If collapsed, expand first before resizing
      ...
      const handleMouseUp = () => {
        ...
        // Restore transitions
        if (sidebarEl) (sidebarEl as HTMLElement).style.transitionDuration = ''
        if (gapEl) gapEl.style.transitionDuration = ''
    ...
    [isMobile, sidebarWidth, setSidebarWidth, state, toggleSidebar],
  )
  ...
  ({ className, ...props }, ref) => {
    return (
      <Input
        ...
        data-sidebar="input"
        className={cn(
          'h-8 w-full bg-input border border-input shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring',
          className,
        )}
        {...props}
      />
...
const SidebarGroup = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(({ className, ...props }, ref) => {
  return <div ref={ref} data-sidebar="group" className={cn('relative flex w-full min-w-0 flex-col px-2 py-1', className)} {...props} />
})
  ...
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn(
          'duration-200 flex h-6 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-hidden ring-sidebar-ring transition-[margin,opa] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const SidebarMenu = React.forwardRef<HTMLUListElement, React.ComponentProps<'ul'>>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    data-sidebar="menu"
    className={cn('flex w-full min-w-0 flex-col gap-1.5 group-data-[collapsible=icon]:gap-1.5', className)}
    {...props}
  />
))
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-[14px] leading-5 outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-9! group-data-[collapsible=icon]:p-2! [&>span]:truncate [&>span]:min-w-0 [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]:[&>svg]:size-5',
  {
    variants: {
      variant: {
      ...
      size: {
        default: 'h-10 md:h-8',
        sm: 'h-8 md:h-7 text-[13px] leading-[18px]',
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<'ul'>>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      'ml-3.5 mr-0 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2 py-0.5',
      'group-data-[collapsible=icon]:hidden',
      className,
    )}
    {...props}
...
>(({ asChild = false, size = 'md', isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        'flex h-9 md:h-7 w-full min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-[14px] leading-5 text-sidebar-foreground outline-hidden ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span]:truncate [&>span]:min-w-0 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground',
        'data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent',

...
export interface ReservationQueryParams {
  ...
  productId?: string
  channel?: string
  channels?: string[] // multi-select (preferred over single `channel`)
  search?: string
}
...
export const reservationService = {
  ...
  async getReservations(venueId: string, params: ReservationQueryParams = {}): Promise<PaginatedReservationsResponse> {
    ...
    // `channels` (multi-select array) takes precedence over single `channel`
  ...
  async getStats(venueId: string, dateFrom: string, dateTo: string): Promise<ReservationStats> {
    const response = await api.get(`/api/v1/dashboard/venues/${venueId}/reservations/stats?dateFrom=${dateFrom}&dateTo=${dateTo}`)
    return response.data
  },
  ...
  async getAvailability(venueId: string, params: AvailabilityQueryParams): Promise<{ date: string; slots: AvailableSlot[] }> {
  ...
  async promoteWaitlist(venueId: string, entryId: string, reservationId: string): Promise<WaitlistEntry> {
    const response = await api.post(`/api/v1/dashboard/venues/${venueId}/reservations/waitlist/${entryId}/promote`, { reservationId })
    return response.data
  },
